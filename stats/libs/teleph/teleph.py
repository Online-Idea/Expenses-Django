...
def add_teleph_calls(data, client_id):
    teleph_calls = []
    client_calls = TelephCall.objects.filter(client_id=client_id)
    ...
    for call in data['content']:
        ...
        if record_exists_check.count() == 0:
            teleph_calls.append(TelephCall(client_id=client_id, datetime=datetime, num_from=num_from,
...
def delete_teleph_calls(from_, to, client_id):
    TelephCall.objects.filter(datetime__gte=from_, datetime__lte=to, client_id=client_id).delete()
